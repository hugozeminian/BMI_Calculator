...
double bmiCalculator(double heightM, double massKg) {
  return (massKg / pow(heightM, 2));
}

double convInchesToFoot(double heightI) { return heightI / 12; }

double convFootToMeter(double heightF) { return heightF / 3.28084; }

double convPoundsToKilogram(double massLbs) { return massLbs / 2.20462; }

int main() {
  double heightF, heightI, massLbs, heightM, massKg;
  ...
  heightF = heightF + convInchesToFoot(heightI);

  heightM = convFootToMeter(heightF);

  massKg = convPoundsToKilogram(massLbs);
  
  cout << fixed;
  cout << setprecision(2);
  
  cout << "Height (m): " << heightM << endl;
  cout << "Weight (kg): " << massKg << endl;
  cout << "BMI: " << bmiCalculator(heightM, massKg) << endl;
